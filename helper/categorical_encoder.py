...
class CategoricalEncoder():
    ...
    def separate_variable_types(self):
        ...
        print('There are {} combined discrete + categorical variables'.format(len(self.categorical)))
    ...
    def rtt_encoding(self, data):
        ...
        self.modality_count_dict = {c: dict(df_cv[c].value_counts()) for c in self.categorical + self.binary}
        rtt_encoding_dict = {}
        for var in self.categorical + self.binary:
            ...
            for modality, count in self.modality_count_dict[var].items():
                targets_in_modality = df_cv[(df_cv[var] == modality) & (df_cv[self.target] == 1)].shape[0]
    ...
    def categorical_encoding_rtt(self):
        ...
        # include binary encoding to final variables
        for var in self.binary:
            le = LabelEncoder().fit(self.df[var].values)
            mapping = dict(zip(le.classes_, range(len(le.classes_))))
            self.encoding_dict_binary[var] = mapping

        # replace binary encoding
        for var in self.binary:
            self.df_encoded[var] = self.df_encoded[var].apply(lambda x: self.encoding_dict_binary[var][x])

        # replace categorical encoding
